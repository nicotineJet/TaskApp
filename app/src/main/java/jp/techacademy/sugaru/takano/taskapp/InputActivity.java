...
public class InputActivity extends AppCompatActivity {
    ...
    private View.OnClickListener mOnDoneClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            addTask();
            //finishメゾットでinputActivityを閉じて前の画面(MainActivity)に戻る
            finish();
        }
    };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //setSupportActionBarメゾットにより、ツールバーをActionBarとして使えるように設定
        setSupportActionBar(toolbar);
        if(getSupportActionBar() != null){
            //setDisplayHomeAsUpEnabledメゾットでActionBarに戻るボタンを表示
        ...
        //EXTRA_TASKが設定されていないとtaskIdには第二引数で指定している規定値「-1」が代入される
        int taskId = intent.getIntExtra(MainActivity.EXTRA_TASK, -1);
        Realm realm = Realm.getDefaultInstance();
        /*mTask = realm.where(Task.class).equalTo("id", taskId).findFirst(); によって、
        Task の id が taskId のものが検索され、
        findFirst() によって最初に見つかったインスタンスが返され、mTask へ代入される
    ...
    private void addTask() {
        ...
        //ActivityのgetSystemServiceメゾットに引数ALARM_SERVICEを与えて取得
